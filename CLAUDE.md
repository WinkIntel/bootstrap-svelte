...
- `vercel-agent-routes.js` — wraps `adapter-static` in `svelte.config.js`: it routes every extension-less path through a Vercel Routing Middleware emitted as an edge function (`.vercel/output/functions/agent-negotiation.func`) and patches `.vercel/output/config.json` (Markdown content types, `404.html` catch-all). `agent-middleware.js` is that middleware (RFC 9110 negotiation via `accept-negotiation.js`, trailing-slash redirects, negotiated 404/406 bodies, `Vary`/`Link` headers); these files and `site-url.js` must stay plain, dependency-free JS because `svelte.config.js` imports them and they are copied verbatim into the function bundle. Inspect the result with `VERCEL=1 pnpm build:site`.
