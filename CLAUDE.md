...
- `vercel-agent-routes.js` — wraps `adapter-static` in `svelte.config.js` and patches `.vercel/output/config.json` with `Accept: text/markdown` rewrites, `Vary`/`Link` headers, and negotiated 404/406 routes. Must stay plain JS (imported by `svelte.config.js`); `negotiate()` is the reference implementation of the routing regexes. Inspect the result with `VERCEL=1 pnpm build:site`.
