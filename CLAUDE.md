...
- `indexnow.js` — the IndexNow key (hosted as `static/<key>.txt`, served at `/<key>.txt`), sitemap parsing, and the submission payload; `pnpm indexnow` (`scripts/indexnow.mjs`) submits the live sitemap's URLs, or the given paths, to `api.indexnow.org`, which forwards them to Bing and the other participating engines. Run it after a production deploy when pages were added, changed, or removed.
