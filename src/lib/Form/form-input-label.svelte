...
<script lang="ts">
    ...
    let { children, class: classValues, elementRef = $bindable(null), isFloating = false, ...restOfProps }: Form.InputLabelProps = $props();

    let classes: string = $derived(uniqueClsx({ 'form-label': !isFloating }, classValues));
</script>
