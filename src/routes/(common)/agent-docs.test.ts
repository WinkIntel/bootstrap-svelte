...
describe('buildAgentsMd', () => {
    ...
    test('is a Markdown document with when-to-use guidance and integration rules', () => {
        ...
        expect(text).toContain('`Modal.RootProps`');
        expect(text).toContain('`svelte ^5.29.0`');
        expect(text).not.toContain('<Card>');
        expect(text).not.toContain('</Card>');
        expect(text).not.toContain('<Modal isShown={showModal}>');
        expect(text).not.toContain('</Modal>');
        expect(text).not.toContain('ButtonProps');
